...
def setup_config(approach, add_noise=False, noise_level=0.2):
    ...
    config = ExperimentConfig(
        approach=approach,
        add_noise=add_noise,  # Explicitly set add_noise
        noise_strategy='fixed',  # Set strategy
        default_noise=noise_level  # Set noise level
    )
...
def run_single_experiment(approach, add_noise=False, noise_level=0.2):
    logging.info(f"\n=== Running {approach.upper()} Experiment ===")
    
    try:
        # Setup configuration with noise parameters
        config = setup_config(approach, add_noise, noise_level)
...
def main():
    ...
    parser.add_argument('--approaches', nargs='+', 
                      default=['aart', 'multitask', 'annotator_embedding'],
